...
class colourIdentifier(Node):
    ...
    def callback(self, data):
        ...
        if (self.isTarget(cv2.bitwise_and(image, image, mask=cv2.bitwise_or(blue_mask, blue_mask)), cv2.bitwise_or(blue_mask, blue_mask))): self.foundTarget[0] = True
...
class GoToPose(Node):
    ...
    def search(self):
        ...
        while not self.foundTarget[0]:
            ...
            time.sleep(TARGET_DETECTION_RATE)

            for i in range(0, int(SEARCH_TIME/TARGET_DETECTION_RATE)):
